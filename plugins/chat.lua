...
api.sendMessage(msg.chat.id, '`Chat Ended`', true) 
end
end
if msg.chat.type == 'private' and chat_info == 'true' then
if blocks[1] == 'end' or blocks[1] == 'chat' then return nil end
api.forwardMessage('-1001048657305', msg.chat.id, msg_id) 
api.sendKeyboard(msg.chat.id, '`Sent!`\n*Wait For Reply*'  ,do_keyboard_endchat(), true)
end
if blocks[1] == 'block' then
if msg.reply and msg.reply.forward_from and msg.chat.type == 'group' and msg.chat.id == -1001048657305 and not blocks[2] then
msg = msg.reply
...
api.sendMessage(user_id, '`- Sorry`\n_You are Blocked_\n\n*Chat ended.*', true) 
 ...
 if msg.chat.type == 'group' and msg.chat.id == -1001048657305 then
...
api.sendMessage(user_id, '_You are UnBlocked_', true) 
    ...
    if msg.reply and msg.reply.forward_from and msg.chat.type == 'group' and msg.chat.id == -1001048657305 then
   msg = msg.reply_to_message
    local receiver = msg.forward_from.id
    local input = blocks[1]
      api.sendMessage(receiver, '*Answer :*'..input, false)
end
end
return {
  action = action,
